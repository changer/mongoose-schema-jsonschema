var schema_jsonSchema = require('./schema');
...
function array_jsonSchema(name) {
  ...
  const caller = this.schema || this.caster;
  if (!caller.jsonSchema) {
    caller.jsonSchema = schema_jsonSchema;
  }
  result.items = caller.jsonSchema(itemName);
